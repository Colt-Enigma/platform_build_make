...
class BlockDifference(object):
  ...
  def WriteScript(self, script, output_zip, progress=None,
    ...
    if not self.src:
      ...
      script.Print(" ")
      script.Print("Flashing ColtOS %s files..." % (self.partition,))
    else:
      script.Print(" ")
      script.Print("Flashing ColtOS %s files after verification." % (self.partition,))
  ...
  def WritePostInstallVerifyScript(self, script):
    partition = self.partition
    script.Print(" ")
    script.Print('Verifying ColtOS %s files...' % (partition,))
    ...
    if self.tgt.extended:
      ...
      script.Print(" ")
      script.Print('Verified ColtOS %s files.' % (partition,))
      if partition == "system":
...
class DynamicPartitionsDifference(object):
  ...
  def _Compute(self):
    ...
    if self._build_realme_vendor:
      ...
      # When building realme vendor, we do not want to override
      # any partition already existing. In this case, we can only
      # resize, but not remove / create / re-create any other
      # partition.
      for p, u in self._partition_updates.items():
